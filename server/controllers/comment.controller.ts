...
export async function likeCommentAction(req: Request, res: Response) {
  const userId = req.user!.id;
  const commentId = getParamResourceId(req, "commentId");

  const comment = await CommentService.likeActionComment(userId, commentId);
  return okRes(null, "Successfully liked comment.", { comment: comment }, res);
}
